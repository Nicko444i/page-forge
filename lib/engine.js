...
export function coerceUPS(raw, fallbackName) {
  ...
  secs = secs.map((s) => {
    ...
    if (s.type !== "product_core" && !ACTIVE_TYPES.includes(s.type)) {
      const body = s.settings && (s.settings.body || s.settings.text) ? (s.settings.body || s.settings.text) : "<p></p>";
      s = { id: s.id, type: "rich_text", settings: { heading: s.settings && s.settings.heading, body } };
    }
    s.settings = s.settings || {};
    if (["benefits", "reviews", "faq"].includes(s.type)) s.blocks = Array.isArray(s.blocks) ? s.blocks.slice(0, 50) : [];
    return s;
  });
