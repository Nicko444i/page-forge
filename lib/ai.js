...
- Long prose / product description => "rich_text" {heading, body}. The "heading" MUST be the ACTUAL title from the source text, keeping its emoji exactly (e.g. "🪒 S10 FoilCare System") — NEVER invent, summarize, or rename it. If a subtitle/tagline sits under that title, put it as the FIRST paragraph of "body" wrapped in <strong> (keep its emoji), then the description paragraph(s) after it.
...
${TYPES_DOC}
- Always capture the opening title, its subtitle/tagline, and the descriptive paragraph as ONE "rich_text" section: heading = the real title exactly as written (with its emoji); body = the subtitle as a bold paragraph (with its emoji) followed by the description. Never drop, rename, or invent these.
- If the text has a short "Top Benefits" / "Key Benefits" list that is SEPARATE from the main detailed benefits, output it as a SECOND "benefits" section right after the main detailed one. Otherwise output a single benefits section.
