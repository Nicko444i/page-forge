...
export function buildShrineTemplate(ups, mainSection) {
  ...
  //    L'AI scrive {heading, body} al PRIMO livello della sezione; versioni
  //    precedenti li mettevano sotto settings. Leggiamo entrambe le forme.
  const rtBody = (x) => String((x && ((x.settings && (x.settings.body || x.settings.text)) || x.body || x.text)) || "");
  const rtHead = (x) => String((x && ((x.settings && (x.settings.heading || x.settings.title)) || x.heading || x.title)) || "");
  const richTexts = (ups.sections || []).filter((x) => x.type === "rich_text" && rtBody(x).trim());
  if (richTexts.length) {
    richTexts.forEach((rt) => { const k = uid("iwt"); sections[k] = imageWithText(rtHead(rt) || name, rtBody(rt), false); order.push(k); });
  } else {
    const subhead = hero && ((hero.settings && hero.settings.subhead) || hero.subhead);
    if (subhead) { const k = uid("iwt"); sections[k] = imageWithText(name, subhead, false); order.push(k); }
  }
